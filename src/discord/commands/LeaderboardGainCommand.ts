...
export default class LeaderboardGainCommand extends BunCommand<CustomClient> {
	...
	constructor() {
		super("gain-leaderboard", {
			description:
				"See the players that gained the most score in a certain time period",
			options: [leaderboardStatOption(true), createTimeOption(false)],
		});
	}
	...
	async sendDataMessage(
		...
		if (!data) {
			data = (await getTopGainsWithin(stat, thenTime)) || {
				total: 0,
				gainData: [],
			};
		}
		...
		const fields: EmbedField[] = gainData
			.slice(startIndex, startIndex + this.ENTIRES_PER_PAGE)
			.map((data, index) => {
				return {
		...
		const response = await interaction.editReply({
			content: "",
			embeds: [embed],
			components: this.buildCategorySelection(
				type,
				time,
				page,
				maxPage,
		...
		this.client
			.await<StringSelectMenuInteraction>(
				`player-gain-leaderboard-category-${id}`,
				1,
			)
			.then(async (select) => {
		...
		this.client
			.await<StringSelectMenuInteraction>(
				`player-gain-leaderboard-time-${id}`,
				1,
			)
			.then(async (select) => {
				if (select.values.length === 0) {
		...
		this.client
			.await<ButtonInteraction>(`player-gain-leaderboard-page-down-${id}`, 1)
			.then(async (button) => {
				await this.sendDataMessage(
					interaction,
					response.id,
		...
		this.client
			.await<ButtonInteraction>(`player-gain-leaderboard-page-up-${id}`, 1)
			.then(async (button) => {
				await this.sendDataMessage(
					interaction,
					response.id,
	...
	buildCategorySelection(
		...
		for (let i = 0; i < 25; i += 25) {
			...
			const builder = new StringSelectMenuBuilder()
				...
				.setMinValues(1)
				.setMaxValues(1)
				.setCustomId(`player-gain-leaderboard-category-${id}`)
		...
		for (let i = 0; i < TIME_OPTIONS.length; i += 25) {
			...
			const builder = new StringSelectMenuBuilder()
				...
				.setMinValues(1)
				.setMaxValues(1)
				.setCustomId(`player-gain-leaderboard-time-${id}`)
		...
		const left = new ButtonBuilder()
			.setCustomId(`player-gain-leaderboard-page-down-${id}`)
			.setLabel("Left")
			.setStyle(ButtonStyle.Primary)
			.setEmoji("⬅️");
		const current = new ButtonBuilder()
			.setCustomId(`player-gain-leaderboard-page-${id}`)
			.setLabel(`Page ${currentPage + 1}/${maxPage}`)
			.setStyle(ButtonStyle.Secondary)
			.setDisabled(true)
			.setEmoji("📄");
		const right = new ButtonBuilder()
			.setCustomId(`player-gain-leaderboard-page-up-${id}`)
			.setLabel("Right")
			.setStyle(ButtonStyle.Primary)
			.setEmoji("➡️");
